...
import com.midnightcoder.krishimitra.ui.viewmodel.ImageDetectionState
    ...
    val imageLauncher = rememberLauncherForActivityResult(
        ...
        uri?.let {
            try {
                ...
                messages.add(
                    ChatMessage(
                        text = "Hey your crop looks very nice ,I am thinking i needs some water and all set.",
                        isUser = false
                    )
                )

//                chatViewModel.detectImage(image)
...
//                chatViewModel.detectImage(image)
    ...
    val imageDetection = chatViewModel.imageDetection.collectAsStateWithLifecycle()

    LaunchedEffect(imageDetection.value) {
        when (val state = imageDetection.value) {
            is ImageDetectionState.Loading -> {
            ...
            is ImageDetectionState.Error -> {
                ...
                messages.add(ChatMessage("❌ Error: ${state.message}", false))
            }

            is ImageDetectionState.Success -> {
                if (messages.lastOrNull()?.text == "typing...") {
                    messages.removeLast()
                }
        ...
        drawerContent = {
            ModalDrawerSheet(
                ...
                Column(
                    ...
                    // Drawer Items
                    DrawerItem(image = R.drawable.winter,"Weather Alerts") { /* Handle click */ }
                    DrawerItem(image = R.drawable.ruppe,"Mandi Prices") { /* Handle click */ }
                    DrawerItem(image = R.drawable.plant,"Fertilisers & Pesticides") { /* Handle click */ }
                    DrawerItem(image = R.drawable.nearby,"Nearby Wholesales") { /* Handle click */ }
                }
            }
        }
    ) {
                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    Column(modifier = Modifier.fillMaxWidth()) {
                        ...
                        PagerIndicator(
                            ...
                            inactiveColor = colorScheme.primary.copy(alpha = 0.5f),
                            indicatorSize = features.size,
                            spacing = 8,
